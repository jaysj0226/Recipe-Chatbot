...
class AskRequest(BaseModel):
    ...
    k: int = Field(default=10, ge=1, le=50, description="검사할 문서 수")
    model: str = Field(default="gpt-4o-mini", description="사용 LLM 모델")
    enable_rewrite: bool = Field(default=True, description="쿼리 보정 사용 여부")
    allow_low_confidence: bool = Field(
        default=False,
        description="유사도/근거가 약할 때도 경고 후 답변 진행 허용 여부",
    )
    decision: Optional[str] = Field(default=None, description="저신뢰 의사결정: proceed|clarify")
    session_id: Optional[str] = Field(default=None, description="세션 ID (옵션)")
