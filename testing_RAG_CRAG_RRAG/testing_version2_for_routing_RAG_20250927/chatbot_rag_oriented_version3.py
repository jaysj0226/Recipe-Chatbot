...
STATIC_DIR = "C:/Users/SunjaeJeong/Desktop/data/files_data/recipe_chatbot_CRAG/static"      # test/static/
...
def query(req: AskReq):
    """프론트엔드 호환성을 위한 /query 엔드포인트 (내부적으로 /ask 호출)"""
    return ask(req)

# ── 4) 로컬 실행 ─────────────────────────────────────────────
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("chatbot_rag_oriented:app", host="127.0.0.1", port=8000, reload=True)
